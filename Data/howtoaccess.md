...
- **API Key Location**: `Data/GenAIkey.md` (in this repository)
- **Authentication**: Use `api-key` header (Azure OpenAI format)
- **API Format**: Azure OpenAI REST API compatible

## API Key

Your API key is stored in `Data/GenAIkey.md` (same directory as this file). The key is a UUID format and should be kept secure.

**Note**: The chatbot server (Lab3) automatically detects and uses this API key if present.
...
**Alternative Endpoint:** `POST /openai/deployments/{model}/chat/completions` (may work if primary fails)
...
**Important**: The model name is included in the URL path, not in the request body for HKBU Gen AI API.
...
# Configuration
API_KEY = "YOUR_API_KEY_HERE"  # Get from Data/GenAIkey.md
...
## Alternative: Kimi (Moonshot AI) API

The project also supports **Kimi API** as an alternative. If `Data/Kimi.md` exists, the chatbot will automatically use Kimi API instead.

**Kimi API Configuration:**
- **Base URL**: `https://api.moonshot.cn/v1`
- **Authentication**: `Authorization: Bearer {API_KEY}` header
- **API Key Location**: `Data/Kimi.md`
- **Endpoint**: `POST /chat/completions`
- **Model in Payload**: Unlike HKBU API, Kimi requires `model` parameter in request body
...
See `howKimi.md` for detailed Kimi API documentation.
...
- **Local Chatbot**: See `Lab3 set up a local chatbot/README.md` for chatbot usage
...
This API is integrated into the local chatbot (Lab3):

- **Location**: `Lab3 set up a local chatbot/scripts/server.py`
- **Auto-detection**: Server automatically uses HKBU Gen AI if `Data/GenAIkey.md` exists
- **Fallback**: Uses Kimi API if `Data/Kimi.md` exists instead
- **Usage**: Start server with `python3 scripts/server.py`, then access `http://localhost:5000`

---

**Last Updated**: December 2025  
**API Status**: ✅ Fully functional and tested  
**File Location**: `math19Dec/Data/howtoaccess.md`
